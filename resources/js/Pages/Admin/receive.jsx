...
const Receive = ({ admin }) => {
    ...
    const [recv_atach_file, setrecv_atach_file] = useState('')
    ...
    const handleFileUpload = (e) => {
        ...
        if (files.length > 0) {
            setrecv_atach_file(e.target.files[0])
        }
    }
    const fetchData = () => {
        axios.post('/api/admin/recive-fetch')
            .then(res => {
    ...
    const FormSubmit = (e) => {
        ...
        formSave.append('recv_name', formData.recv_name)
        formSave.append(' admin_id', admin_id)
        formSave.append('admin_type', admin_type)
        formSave.append('recv_ref_no', formData.recv_ref_no)
        formSave.append('recv_address', formData.recv_address)
        formSave.append('recv_note', formData.recv_note)
        formSave.append('recv_form_title', formData.recv_form_title)
        formSave.append('recv_atach_file', recv_atach_file)
        formSave.append('recv_date', formData.recv_date)



        axios.post('/api/admin/recive-store', formSave,)
            .then(response => {
    ...
    const DeleteData = (e, id) => {
        ...
        axios.post(`/api/admin/recive-delete/${id}`)
            .then(response => {
                console.log(response)
                fetchData();
            })
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {
                                    data.map((datas, idx) => (
                                        <tr>
                                            ...
                                            <td className='px-6 py-3 text-left text-xs'>{datas.recv_date}</td>
                                            <td className='px-6 py-3 text-left text-xs'>{datas.recv_form_title}</td>
                                            <td className='px-6 py-3 text-left text-xs'>{datas.recv_address}</td>
                                            <td className='px-6 py-3 text-left text-xs'>{datas.recv_note}</td>
                                                        ...
                                                        <div className="modal-body">
                                                            ...
                                                            <form action className="w-full grid grid-cols-2 gap-5 px-6 mt-10 relative">
                                                                <div className="form-group w-full ">
                                                                    ...
                                                                    <input onChange={handleFormdata} name='recv_name' value={formData.recv_name} type="text" className="w-full border-gray-300" />
                                                                </div>
                                                                <div className="form-group w-full ">
                                                                    <label htmlFor>Reference no</label>
                                                                    <input onChange={handleFormdata} name='recv_ref_no' value={formData.recv_ref_no} type="text" className="w-full border-gray-300" />
                                                                </div>
                                                                <div className="form-group w-full">
                                                                    <label htmlFor> Date</label> <br />
                                                                    <input onChange={handleFormdata} name='recv_date' value={formData.recv_date} type="date" className="w-full border-gray-300" />
                                                                </div>
                                                                <div className="form-group w-full">
                                                            ...
                                                            <div className="form-group w-full px-6 mt-3">
                                                                ...
                                                                <textarea onChange={handleFormdata} value={formData.recv_address} name='recv_address' type="date" className="w-full border-gray-300" ></textarea>
                                                            </div>
                                                            <div className="form-group w-full px-6 mt-3">
                                                                <label htmlFor>Note</label> <br />
                                                                <textarea onChange={handleFormdata} value={formData.recv_note} name='recv_note' type="date" className="w-full border-gray-300" ></textarea>
                                                            </div>
                                                            <div className="form-group w-full px-6">
                                                                ...
                                                                <div class="flex items-center justify-center w-full">
                                                                    <label for="dropzone-file" class="flex flex-col items-center justify-center w-full h-[2.5rem] border-2 border-gray-300 border-dashed  cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                                                                        ...
                                                                        <input name='recv_atach_file' onChange={handleFileUpload} id="dropzone-file" type="file" class="hidden" />
                                                                    </label>
                                                                </div>
                                                            </div>
                                                        </div>
